...
export let options = {
    scenarios: {
        seat_lookup: {
            ...
            startRate: 100, // 초당 100개 요청 시작
            timeUnit: '1s', // 초 단위로 TPS 유지
            preAllocatedVUs: 200, // 미리 200명의 VU 할당
            maxVUs: 500, // 최대 500명의 VU까지 확장 가능
            startTime: '15s', // setup() 실행 후 15초 뒤에 시작
            stages: [
                { duration: '30s', target: 100 }, // 30초 동안 100 TPS 유지
                { duration: '1m', target: 200 },  // 1분 동안 200 TPS 유지
                { duration: '1m', target: 250 },  // 1분 동안 250 TPS 유지
                { duration: '30s', target: 200 }, // 30초 동안 200 TPS 유지
